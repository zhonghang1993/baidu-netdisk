...
public class StsService {
    ...
    public StsInfo getStsInfo(){
        StsInfo stsInfo = storageDaoI.getStsInfo();
        //可能会过期？
        if(stsInfo == null || (stsInfo.getExpirationSecond() - DateUtil.currentSeconds()) < 0){
            stsInfo = stsToken();
            storageDaoI.saveStsInfo(stsInfo);
        }
     ...
     * @return
     */
    private synchronized StsInfo stsToken() {
        OrganizationInfo organizationInfo = organizationInfoService.getOrganizationInfo();
